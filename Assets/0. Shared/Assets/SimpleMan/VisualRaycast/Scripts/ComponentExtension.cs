...
namespace SimpleMan.VisualRaycast {
    public static class ComponentExtension {
        ...
        //NOTE: 追記
        public static CastResult Raycast(this Transform component, bool castAll, Vector3 originPoint, Vector3 direction, float maxDistance, LayerMask mask, bool ignoreSelf = true) {
            return MakeRaycast(component, new Ray(originPoint, direction), maxDistance, mask, castAll, ignoreSelf);
        }

        private static CastResult MakeRaycast(Transform sender, Ray ray, float maxDistance, LayerMask mask, bool castAll, bool ignoreSelf) {
            RaycastHit[] hits = Physics.RaycastAll(ray, maxDistance, mask);
            CastResult result = CalculateResult(hits, sender, !castAll, ignoreSelf);
        ...
        //NOTE: 追記
        public static CastResult Boxcast(this Transform component, bool castAll, Vector3 originPoint, Vector3 direction, Vector3 boxSize, float maxDistance, LayerMask mask, bool ignoreSelf = true) {
            return MakeBoxcast(component, new Ray(originPoint, direction), maxDistance, boxSize, component.rotation, mask, castAll, ignoreSelf);
        }

        private static CastResult MakeBoxcast(Transform sender, Ray ray, float maxDistance, Vector3 size, Quaternion rotation, LayerMask mask, bool castAll, bool ignoreSelf) {
            RaycastHit[] hits = Physics.BoxCastAll(ray.origin, size * 0.5f, ray.direction, rotation, maxDistance, mask);
            CastResult result = CalculateResult(hits, sender, !castAll, ignoreSelf);
        ...
        //NOTE: 追記
        public static CastResult SphereCast(this Transform component, bool castAll, Vector3 originPoint, Vector3 direction, float radius, float maxDistance, LayerMask mask, bool ignoreSelf = true) {
            return MakeSpherecast(component, new Ray(originPoint, direction), maxDistance, radius, mask, castAll, ignoreSelf);
        }

        private static CastResult MakeSpherecast(Transform sender, Ray ray, float maxDistance, float radius, LayerMask mask, bool castAll, bool ignoreSelf) {
            RaycastHit[] hits = Physics.SphereCastAll(ray, radius, maxDistance, mask);
            CastResult result = CalculateResult(hits, sender, !castAll, ignoreSelf);
        ...
        //NOTE: 追記
        public static Collider[] BoxOverlap(this Transform sender, Vector3 center, Vector3 boxSize, Quaternion rotation, LayerMask layerMask, bool ignoreSelf = true) {
            return MakeBoxOverlap(sender, center, boxSize, rotation, layerMask, ignoreSelf);
        }
        private static Collider[] MakeBoxOverlap(Transform sender, Vector3 center, Vector3 boxSize, Quaternion rotation, LayerMask layerMask, bool ignoreSelf) {
            Collider[] colliders = Physics.OverlapBox(center, boxSize * 0.5f, rotation, layerMask);
            CalculateOverlapResult(ref colliders, sender, ignoreSelf);
        ...
        //NOTE: 追記
        private static CastResult CalculateResult(RaycastHit[] hits, Transform sender, bool fistHitOnly, bool ignoreSelf) {
            LinkedList<RaycastHit> hitsList = new LinkedList<RaycastHit>(hits);
            hits = null;
            ...
            if (ignoreSelf) {
                ...
                foreach (var item in hitsList) {
                    itemTransform = item.transform;
                    if (itemTransform == sender || itemTransform.IsChildOf(sender))
                        toRemove.Add(item);
                }
        ...
        private static void CalculateOverlapResult(ref Collider[] hits, Transform sender, bool ignoreSelf) {
            if (ignoreSelf) {
                LinkedList<Collider> hitsList = new LinkedList<Collider>(hits);
                List<Collider> selfHits = new List<Collider>();
                hits = null;
