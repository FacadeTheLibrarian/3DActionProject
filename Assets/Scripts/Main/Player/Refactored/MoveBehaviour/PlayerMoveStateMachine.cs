...
internal sealed class PlayerMoveStateMachine : MonoBehaviour {
    ...
    public void Initialization(in PlayerInputs inputs, in AnimationStateController animationStateController, in PlayerDirection direction, in MonsterData monsterData, in PlayerStamina stamina) {
        _playerHorizontalMove.Initialization(animationStateController, direction);
        _playerDodge.Initialization(direction, stamina, monsterData);
    ...
    public void OnStartDodge(InputAction.CallbackContext context) {
        ...
        if (_playerStamina.IsRunOutOfStamina) {
            return;
        }
        _animationStateController.SetSpeedImmidiately(0.0f);
